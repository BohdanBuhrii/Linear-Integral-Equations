...
\newcommand{\boundprob}{(\ref{laplace-eq}) -- (\ref{neumann-condition})} 
 ...
 \theorem{single-sol} Нехай $\Gamma_{1}, \Gamma_{2}$ -- гладкі границі, що належать класу $C^1$, обмежують двозв'язну область $D$. Тоді задача \boundprob \space має на $D$ не більше одного розв'язку.
 ...
 \hspace{0.45cm} \defn{harminic-func} Нехай $D \subset \mathbb{R}^2$ -- замкнена обмежена облясть. Функція $u \in C^2(D)$, що набуває дійсних значень, називається гармонічною, якщо вона задовольняє рівняння Лапласа $\delta u = 0$ в $D$.
 
 \defn{fundamental-solution}  Функція
 $$
 ...
 \theorem{potential-on-bound} Нехай $\partial D$ належить класу $C^{2}$ і $\varphi \in C(\partial D) .$ Тоді потенціал простого шару $u$ з густиною $\varphi$ неперервний на $\mathbb{R}^{2} .$ На границі області справджується рівність
 $$
 ...
 \theorem{unique-solution} Для $\Gamma_{i} \in C^{2}$ i $f_{i} \in C\left(\Gamma_{i}\right)$ , $i=1,2,$ система інтегралъних рівнянь (\ref{IE-system}) має единий розв'язок $\mu_{i} \in C\left(\Gamma_{i}\right), i=1,2$, який неперервно залежить від вхідих даних.
 
 Доведення: Введемо наступні інтегральні оператори:
 $$
 \begin{array}{l}
 	
 	\displaystyle
 	(A \varphi)(x):= 2 \intl_{\Gamma_{1}} \Phi(x, y) \varphi(y)  d s(y), \quad x \in \Gamma_{1}
 	\\[0.7cm]
 	 
 	 \displaystyle
 	(B \varphi)(x):= 2 \intl_{\Gamma_{2}} \Phi(x, y) \varphi(y)  d s(y), \quad x \in \Gamma_{1}
 	\\[0.7cm]
	...
	\\[0.7cm]
 	 ...
 	 \displaystyle
	(D \varphi)(x):= 2 \intl_{\Gamma_{1}} \frac{\partial \Phi(x, y)}{\partial \nu(x)} \varphi(y)  d s(y), \quad x \in \Gamma_{2}
	\\[0.7cm]
 \end{array}
 $$
 Запишемо систему інтегральних рівнянь \ref{IE-system} в операторному вигляді:
 \begin{equation}
 	\label{IE-operator}
	\varphi-U \varphi=F
\end{equation}

 де $\varphi_:=\left(\varphi_{1}, \varphi_{2}\right)^{T}, F:=\left(2 f_{1}, 2 f_{2}\right)^{T}$ і оператор $U$ заданий як
 $$
 ...
 \end{array}\right)
 $$
 Позначимо $\Gamma:=\Gamma_{1} \cup \Gamma_{2}$. Оператори $B$ та $C$ містять неперервні ядра, отже $\epsilon$ компактними в $C(\Gamma) .$  Оператори А та D містять слабосингулярні ядра, а отже також є компактними. Таким чином ми можемо застосувати теорію Рісса для доведення коректності нашої системи IP.
  
  Відомо, що простір нулів $N(I-U)=\{0\}$, тобто оператор $(I-U)$ ін'єктивний. Тоді згідно теорії Рісса $(I-U)$ сюр'єктивний, тобто
  $$
  \forall F \in C(\Gamma) \quad \exists ! \varphi \in C(\Gamma): \quad(I-U) \varphi=F
  $$
  Тоді, обернений оператор $(I-U)^{-1}$ обмежений і розв'язок операторного рівняння \ref{IE-operator} можа подати у вигляді
  $$
  \varphi=(I-U)^{-1} F
  $$
