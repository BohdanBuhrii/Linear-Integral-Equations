...
\newpage
Ядро $K_{22}$ можна записати у вигляді

$$
\begin{array}{l}
	\displaystyle
	K_{11} (t, \tau) = {K_{11}}^{(1)}  (t, \tau) \ln{\left( \frac{4}{e} \sin^{2}{\frac{t - \tau}{2}}   \right) }  +  {K_{11}}^{(2)} (t, \tau)
	\\ [1cm]
	
	\displaystyle
	{K_{11}}^{(1)} (t, \tau) = - \frac{1}{2} \abs{x'_{2}(\tau) }; 
	\\ [1cm]
	
	\displaystyle
	{K_{11}}^{(2)} (t, \tau) = \frac{1}{2} \abs{x'_{2}(\tau) } \ln{\frac{\frac{4}{e} \sin^{2}{\frac{t - \tau}{2} }}  { \abs{x_{2}(t) - x_{2}(\tau)}^2 }}
	\\ [1cm]	
\end{array}
$$
